...
public class Main {
	...
	public static void test1() {
		 ...
		 * 情况1：所谓基本语句就是指它的执行体执行次数最多的语句，而语句分为三大类顺序语句、条件语句、循环语句，
		 ...
		 * 因为它们执行体的执行次数都是1】
		 * 情况2：算法中有多个并列的循环时，那该算法的基本语句就有多条了，即每一条并列的循环语句都是一条基本语句
		 * 情况3：当然当算法中有递归语句时，那我们其实就不用关心基本语句是谁了，我们直接跳到第二步看递归函数会被调用多少次即可
		 * 
		 * 第二步：计算基本语句执行体的执行次数
		 * 
		 ...
		 * 
		 * 对应第一步的情况2：算法中有多个并列的循环时，那么该算法的执行数量级 = 所有基本语句执行体执行数量级之和
		 */
	}
	...
	/*
	 * 第一步：发现有多个并列的循环语句
	 * 所以该算法的基本语句有两条：第一个for循环和第二个嵌套循环的内层for循环
	 * 
	 * 第二步：执行次数
	 * 第一条基本语句循环体的执行次数为n，第二条基本语句循环体的执行次数为n ^ 2
	 * 
	 * 第三步：执行数量级
	 * 第一条基本语句循环体的执行数量级为n，第二条基本语句循环体的执行次数为n ^ 2，
	 * 因此该算法的执行量级为两者相加n^2 + n，没有常数可省略、可以省略低阶、最高阶没有系数可省略，保留最高阶n^2即为该算法的执行数量级
	 * 所以最终该算法的时间复杂度为平方级：O(n^2)
	 */
	...
	/*
	 * 第一步：发现有多个并列的循环语句
	 * 所以该算法的基本语句有两条：第一个for循环和第二个for循环
	 * 
	 * 第二步：执行次数
	 * 第一条基本语句循环体的执行次数为n，第二条基本语句循环体的执行次数为k
	 * 
	 * 第三步：执行数量级
	 * 第一条基本语句循环体的执行数量级为n，第二条基本语句循环体的执行次数为k
	 * 因此该算法的执行量级为两者相加n + k，没有常数可省略、没有低阶可省略、最高阶没有系数可省略，保留最高阶n + k即为该算法的执行数量级
	 * 所以最终该算法的时间复杂度为：O(n + k)，同时由于大O表示法描述的事n和k趋于无穷时的情况，所以可以把n和k看做是相等的——即2n或2k，所以该算法的
	 * 复杂度其实还是个线性级：O（n)
	 */
